...
#-------------------------------------------------------------------------------
#---------------- section for second dataset derived from first ----------------
#-------------------------------------------------------------------------------
...
def generate_new_order(order_line):
  if order_line[1] == "":
    ...
    order_line_id = order_line[0] + order_line[1] # combination of order_id and line_number into one field
    article_id = INVERTED_PRICES[order_line[4]]   # new article_id is position of the price in the PRICES list
    order_date = order_line[2]                    # order_date kept the same
    country = REGION_COUNTRIES[order_line[3]]     # country adapted according to REGION_COUNTRIES mapping
    sales_price = order_line[5]                   # sales_price is discounted_price before VAT
    new_order = [[
      order_line_id,
      str(article_id),
      order_date,
...
#-------------------------------------------------------------------------------
#--------------- section for executing derivation and persisting ---------------
#-------------------------------------------------------------------------------
